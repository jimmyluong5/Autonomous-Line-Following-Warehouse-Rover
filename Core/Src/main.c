...
int main(void) {
  ...
  while (1) {
    ...
    if (UART_CONTROL_GetMode() == UART_MODE_MOTOR &&
        (HAL_GetTick() - last_display_time >= 250)) {
...
static void MX_GPIO_Init(void) {
  ...
  HAL_GPIO_WritePin(GPIOA,
                    DC_Motor_BIN2_Pin | DC_Motor_BIN1_Pin | DC_Motor_STBY_Pin |
                        DC_Motor_AIN2_Pin | DC_Motor_AIN1_Pin | Stepper_DIR_Pin,
                    GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, Stepper_ENABLE_Pin | RFID_RST_Pin | RFID_SPI_CS_Pin,
                    GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(ADC_SPI_CS_GPIO_Port, ADC_SPI_CS_Pin, GPIO_PIN_SET);

  /*Configure GPIO pins : DC_Motor_BIN2_Pin DC_Motor_BIN1_Pin DC_Motor_STBY_Pin
     DC_Motor_AIN2_Pin DC_Motor_AIN1_Pin Stepper_DIR_Pin Stepper_ENABLE_Pin */
  GPIO_InitStruct.Pin = DC_Motor_BIN2_Pin | DC_Motor_BIN1_Pin |
                        DC_Motor_STBY_Pin | DC_Motor_AIN2_Pin |
                        DC_Motor_AIN1_Pin | Stepper_DIR_Pin |
                        Stepper_ENABLE_Pin;
