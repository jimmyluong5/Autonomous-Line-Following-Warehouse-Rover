...
void stepper_move_steps(int32_t steps, uint32_t speed) {
  ...
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, (arr + 1) / 2); // 50% duty
                                                               // cycle.
  ...
  HAL_GPIO_WritePin(Stepper_ENABLE_GPIO_Port, Stepper_ENABLE_Pin,
                    GPIO_PIN_RESET);
  ...
  for (int32_t i = 0; i < steps; i++) {
    ...
    __HAL_TIM_CLEAR_FLAG(&htim1, TIM_FLAG_UPDATE);
    (void)htim1.Instance
        ->SR; // Force bus synchronization to prevent double-triggering
  }
...
// Rotate the output shaft by a specific angle in degrees (positive = CW,
// negative = CCW)
void stepper_move_degrees(float degrees, uint32_t speed) {
  ...
  int32_t steps =
      (int32_t)(calculated_steps >= 0.0f ? (calculated_steps + 0.5f)
                                         : (calculated_steps - 0.5f));
